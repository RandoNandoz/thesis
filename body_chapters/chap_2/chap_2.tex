...
%       \caption[Short Caption to Appear in List of Figures.]{Longer caption to appear in the main body of
% the text below (or above) the actual figure.}
%       \label{fig:little_house_on_the_swiss_prairie}
%   \end{center}
% \end{figure}

% Figures can be placed with the \textbf{\textbackslash figure} environment. Using the option \textit{h} in
% the optional arguments is useful to
% 'lock' figures in the place you want them to, otherwise Latex will place it where it sees fit.

% Use the \textbf{\textbackslash label} command to generate labels; these act as handles which you can refer
% to in other parts of your work with the
% \textbf{\textbackslash ref} command.

% Notice the \textbf{short caption} and \textbf{long caption} and in the \textbf{\textbackslash caption}
% command within the \textbf{\textbackslash
% figure} environment. The List of Tables/Figures pages will use the short caption while the longer caption
% appears with the figure.
...
State-based capture involves saving $s_{pre}$, typically serializing $s_{pre}$ to disk, then de-serializing
...
de-serialization. Currently, the most robust
Python serialization library which we elected to use, dill \cite{dilldocs} do not support generator,
traceback, and code frame objects. Code frame
...
a meaningful value when de-serialized,
