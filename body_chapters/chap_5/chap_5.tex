...
Here we interpret some results of the RQs.
...
correctly exercised the function-under-test; we were unable to find any examples where the default external state
...
may necessitate the usage of action-based carving to resolve this issue in future work.
...
approach that involved generating assertions by recording and inspecting the entire program state.
...
alongside standard debugging procedures. Additionally, the tool uses the \verb|sys.monitoring| API instead
...
however, for the BugsInPy benchmarks, we already knew the buggy functions, where a naive user who had never used ExploTest
...
This adds a challenge to interpreting the results of RQ1. Future work should compare different unit testing techniques,
...
We suggest an approach closer to Auger's \cite{augerblogspot}, where we capture parts of the
...
The approach used in the tool to generate oracles is naive. It assumes the oracle for the test is simply the output of the function, however, for many functions, especially methods
...
Synthesizing all our RQ answers, we find ExploTest, and consequently static, unit test carving in Python is a sufficiently
effective tool to seed unit test suites with behaviour carved from system tests. It is best used when the system test is already
expensive, consistent with the formative test factoring work by Saff \cite*{saff2005automatic,saff2004mock}. Investigating cognitive
and developer performance overhead, we find a proposed usage of the tool overlaps with standard systematic debugging procedure,
meaning ExploTest can be integrated into existing developer workflows with little friction.

The oracle generation problem described by \cite{barr2014oracle} still persists, despite the rich integration test input.
Therefore, this style of state-based
carving is viable as a byproduct of debugging, capturing bug-triggering input surfaced during bug localization which
may assist with seeding a unit test suite with inputs. We advise avenues for future work: improved oracle generation to
solve the oracle problem, via program state instrumentation like Pythoscope, or LLM-guided oracle generation; improved
argument recreation through action-based carving; a less targeted runner that carves all tests like those found in ARTISAN \cite{gambi2023action}.